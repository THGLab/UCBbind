...
def sequence_to_database(sequence, fasta_output, queryDB):
    ...
    seq = Seq(sequence)  # Create the sequence
...
def similar_protein_search(queryDB, refDB, resultDB, tmp):
...
def get_similar_proteins(query_seq, ref_df, config, threshold):
    sequence_to_database(query_seq, config.fasta_file, config.queryDB)
    similar_protein_search(config.queryDB, config.refDB, config.resultDB, config.tmp)
    count, sim_sequences = parse_results(config.output_file, threshold, ref_df)
